...
exports.createSelection = async (req, res) => {
    try {
        ...
        if (!type || !['wedding-dress', 'vest', 'bride-engage', 'groom-engage', 'tone-color', 'wedding-venue', 'wedding-theme'].includes(type)) {
            return res.status(400).json({
                success: false,
                message: 'Type is required and must be "wedding-dress", "vest", "bride-engage", "groom-engage", "tone-color", "wedding-venue", or "wedding-theme"'
            });
        }
        ...
        if (type === 'wedding-dress') {
            ...
            const {
                vestStyleIds, vestColorIds, vestMaterialIds,
                vestLapelIds, vestPocketIds, vestDecorationIds
            } = req.body;
            ...
            const {
                brideEngageStyleIds, brideEngageMaterialIds,
                brideEngagePatternIds, brideEngageHeadwearIds
            } = req.body;
            ...
            const {
                groomEngageOutfitIds, groomEngageAccessoryIds
            } = req.body;
        ...
        if (type === 'wedding-dress') {
            ...
            await selection.populate([
                'vestStyles', 'vestColors', 'vestMaterials',
                'vestLapels', 'vestPockets', 'vestDecorations'
            ]);
        } else if (type === 'bride-engage') {
            await selection.populate([
                'brideEngageStyles', 'brideEngageMaterials',
                'brideEngagePatterns', 'brideEngageHeadwears'
            ]);
        } else if (type === 'groom-engage') {
            await selection.populate([
                'groomEngageOutfits', 'groomEngageAccessories'
            ]);
        } else if (type === 'tone-color') {
...
exports.createAlbum = async (req, res) => {
    try {
        ...
        if (!type || !['wedding-dress', 'vest', 'bride-engage', 'groom-engage', 'tone-color', 'wedding-venue', 'wedding-theme'].includes(type)) {
            return res.status(400).json({
                success: false,
                message: 'Type is required and must be "wedding-dress", "vest", "bride-engage", "groom-engage", or "tone-color"'
            });
        }
        ...
        if (type === 'wedding-dress') {
            ...
            await currentSelection.populate(['weddingToneColors', 'engageToneColors']);
        } else if (type === 'wedding-venue') {
        ...
        if (type === 'wedding-dress') {
            ...
            newSelectionData = {
                ...
                vestMaterials: currentSelection.vestMaterials.map(material => material._id),
                vestLapels: currentSelection.vestLapels.map(lapel => lapel._id),
                vestPockets: currentSelection.vestPockets.map(pocket => pocket._id),
                vestDecorations: currentSelection.vestDecorations.map(decoration => decoration._id)
            };
        } else if (type === 'bride-engage') {
            newSelectionData = {
                ...newSelectionData,
                brideEngageStyles: currentSelection.brideEngageStyles.map(style => style._id),
                brideEngageMaterials: currentSelection.brideEngageMaterials.map(material => material._id),
                brideEngagePatterns: currentSelection.brideEngagePatterns.map(pattern => pattern._id),
                brideEngageHeadwears: currentSelection.brideEngageHeadwears.map(headwear => headwear._id)
            };
        } else if (type === 'groom-engage') {
            newSelectionData = {
                ...newSelectionData,
                groomEngageOutfits: currentSelection.groomEngageOutfits.map(outfit => outfit._id),
                groomEngageAccessories: currentSelection.groomEngageAccessories.map(accessory => accessory._id)
            };
        } else if (type === 'tone-color') {
            newSelectionData = {
                ...newSelectionData,
                weddingToneColors: currentSelection.weddingToneColors.map(color => color._id),
                engageToneColors: currentSelection.engageToneColors.map(color => color._id)
            };
        } else if (type === 'wedding-venue') {
            newSelectionData = {
                ...newSelectionData,
